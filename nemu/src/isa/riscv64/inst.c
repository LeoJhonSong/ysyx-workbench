...
static int decode_exec(Decode *s) {
    ...
    // INSTPAT(L"│????????????      │?????│000   │?????      │0000011│", I, rd = SEXT(Mr(rs1 + imm, 1), 1));        // lb
    // INSTPAT(L"│????????????      │?????│001   │?????      │0000011│", I, rd = SEXT(Mr(rs1 + imm, 2), 2));        // lh
    // INSTPAT(L"│????????????      │?????│010   │?????      │0000011│", I, rd = SEXT(Mr(rs1 + imm, 4), 4));        // lw
    ...
    INSTPAT(L"│????????????????????           │?????      │1101111│", J, rd = s->pc + 4; s->dnpc = s->pc + imm; ERROR("test\n")); // jal
    ...
    // INSTPAT(L"│0000000     │?????│?????│001   │?????      │0111011│", R, rd = SEXT(BITS(rs1, 31, 0) >> rs2, 32)); // srlw
    // I-type │imm[11:0]         │rs1  │funct3│rd         │opcode │
