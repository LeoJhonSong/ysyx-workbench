...
static int decode_exec(Decode *s) {
    ...
    INSTPAT(L"│0000000?????      │?????│001   │?????      │0011011│", I, rd = SEXT(BITS(rs1 << imm, 31, 0), 32));                       // slliw
    INSTPAT(L"│0000000?????      │?????│101   │?????      │0011011│", I, rd = SEXT(BITS((uint64_t)rs1 >> imm, 31, 0), 32));             // srliw // CHECK: zero-extend
    INSTPAT(L"│0100000?????      │?????│101   │?????      │0011011│", I, rd = SEXT(BITS((int32_t)BITS(rs1, 31, 0) >> imm, 31, 0), 32)); // sraiw // CHECK: msb-extend
    // S-type │imm[11:5]   │rs2  │rs1  │funct3│imm[4:0]   │opcode │
