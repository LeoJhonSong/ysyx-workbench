...
static int decode_exec(Decode *s) {
    ...
    INSTPAT(L"│0000000?????      │?????│001   │?????      │0011011│", I, rd = (uint64_t)BITS(rs1 << BITS(imm, 4, 0), 31, 0));                       // slliw // CHECK: shamt = imm[4:0]
    INSTPAT(L"│0000000?????      │?????│101   │?????      │0011011│", I, rd = (uint64_t)BITS((uint64_t)rs1 >> BITS(imm, 4, 0), 31, 0));             // srliw // CHECK: zero-extend, shamt = imm[4:0]
    INSTPAT(L"│0100000?????      │?????│101   │?????      │0011011│", I, rd = (uint64_t)((int32_t)BITS(rs1, 31, 0) >> BITS(imm, 4, 0))); // sraiw // CHECK: msb-extend, shamt = imm[4:0]
    // S-type │imm[11:5]   │rs2  │rs1  │funct3│imm[4:0]   │opcode │
