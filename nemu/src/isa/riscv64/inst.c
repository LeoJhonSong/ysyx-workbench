...
static int decode_exec(Decode *s) {
    ...
    // INSTPAT(L"│????????????      │?????│000   │?????      │0000011│", I, rd = BITS(Mr(rs1 + imm, 1), 7, 0));        // lb
    // INSTPAT(L"│????????????      │?????│001   │?????      │0000011│", I, rd = BITS(Mr(rs1 + imm, 2), 15, 0));        // lh
    INSTPAT(L"│????????????      │?????│010   │?????      │0000011│", I, rd = BITS(Mr(rs1 + imm, 4), 31, 0));        // lw
    ...
    INSTPAT(L"│????????????      │?????│001   │?????      │0010011│", I, rd = rs1 << BITS(imm, 4, 0));                        // slli
    ...
    // INSTPAT(L"│0100000?????      │?????│101   │?????      │0010011│", I, rd = rs1 >> SEXT(BITS(imm, 4, 0), 5));  // rsai
    INSTPAT(L"│000000000001      │00000│000   │00000      │1110011│", I, NEMUTRAP(s->pc, R(10)));                // ebreak, Note: R(10) is $a0
    // S-type │imm[11:5]   │rs2  │rs1  │funct3│imm[4:0]   │opcode │
    // INSTPAT(L"│???????     │?????│?????│000   │?????      │0100011│", S, Mw(rs1 + imm, 1, rs2)); // sb
    // INSTPAT(L"│???????     │?????│?????│001   │?????      │0100011│", S, Mw(rs1 + imm, 2, rs2)); // sh
    // INSTPAT(L"│???????     │?????│?????│010   │?????      │0100011│", S, Mw(rs1 + imm, 4, rs2)); // sw
    // B-type │imm[12|10:5]│rs2  │rs1  │funct3│imm[4:1|11]│opcode │
    INSTPAT(L"│???????     │?????│?????│000   │?????      │1100011│", B, if (rs1 == rs2) { s->dnpc = s->pc + imm; }); // beq
    INSTPAT(L"│???????     │?????│?????│001   │?????      │1100011│", B, if (rs1 != rs2) { s->dnpc = s->pc + imm; ERROR("pc: %lx, snpc: %lx, dnpc: %lx\n", s->pc, s->snpc, s->dnpc);}); // bne
    ...
    // INSTPAT(L"│0000000     │?????│?????│001   │?????      │0111011│", R, rd = SEXT(BITS(rs1 << rs2, 31, 0), 64)); // sllw
    // INSTPAT(L"│0000000     │?????│?????│001   │?????      │0111011│", R, rd = SEXT(BITS(rs1, 31, 0) >> rs2, 64)); // srlw
    // I-type │imm[11:0]         │rs1  │funct3│rd         │opcode │
