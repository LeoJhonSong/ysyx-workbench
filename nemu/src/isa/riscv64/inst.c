...
static void decode_operand(Decode *s, int64_t **rd_pptr, int64_t **rs1_pptr, int64_t **rs2_pptr, int64_t *imm_ptr, int type) {
    ...
    switch (type) {
        ...
        case TYPE_U:
            *rd_pptr = (int64_t *)&R(rd);
            *imm_ptr = SEXT(BITS(i, 31, 12) << 12, 32); // sext(imm[31:12]) << 12
            break;
        case TYPE_J:
            *rd_pptr = (int64_t *)&R(rd);
...
static int decode_exec(Decode *s) {
    ...
    INSTPAT(L"│????????????      │?????│000   │?????      │1100111│", I, s->dnpc = (rs1 + imm) &~ 1; rd = s->pc + 4;);         // jalr
    ...
    INSTPAT(L"│0000000?????      │?????│101   │?????      │0011011│", I, rd = SEXT(BITS(rs1, 31, 0) >> BITS(imm, 4, 0), 32));  // srliw // CHECK: zero-extend, shamt = imm[4:0]
    INSTPAT(L"│0100000?????      │?????│101   │?????      │0011011│", I, rd = (int64_t)SEXT(BITS(rs1, 31, 0), 32) >> BITS(imm, 4, 0)); // sraiw // CHECK: msb-extend, shamt = imm[4:0]
    // S-type │imm[11:5]   │rs2  │rs1  │funct3│imm[4:0]   │opcode │
