...
static int decode_exec(Decode *s) {
    ...
    INSTPAT(L"│????????????      │?????│000   │?????      │0000011│", I, rd = SEXT(BITS(Mr(rs1 + imm, 1), 7, 0), 64));  // lb
    INSTPAT(L"│????????????      │?????│001   │?????      │0000011│", I, rd = SEXT(BITS(Mr(rs1 + imm, 2), 15, 0), 64)); // lh
    INSTPAT(L"│????????????      │?????│010   │?????      │0000011│", I, rd = SEXT(BITS(Mr(rs1 + imm, 4), 31, 0), 64)); // lw
    ...
    INSTPAT(L"│???????     │?????│?????│101   │?????      │1100011│", B, int64_t a, b; a = rs1; b = rs2; if (a >= b) { s->dnpc = s->pc + imm; }); // bge
