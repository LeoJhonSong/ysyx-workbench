...
static int decode_exec(Decode *s) {
    ...
    INSTPAT(L"│????????????      │?????│100   │?????      │0000011│", I, rd = BITS(Mr(rs1 + imm, 1), 7, 0));                 // lbu
    INSTPAT(L"│????????????      │?????│101   │?????      │0000011│", I, rd = BITS(Mr(rs1 + imm, 2), 15, 0));                 // lhu
    ...
    INSTPAT(L"│010000??????      │?????│101   │?????      │0010011│", I, rd = rs1 >> BITS(imm, 4, 0));           // srai
    ...
    INSTPAT(L"│????????????????????           │?????      │0110111│", U, printf("%lx\n", imm << 12); rd = (uint64_t)imm << 12);   // lui
    ...
    INSTPAT(L"│0100000?????      │?????│101   │?????      │0011011│", I, rd = SEXT(BITS(rs1 >> imm, 31, 0), 32)); // sraiw
    // S-type │imm[11:5]   │rs2  │rs1  │funct3│imm[4:0]   │opcode │
