...
static int decode_exec(Decode *s) {
    ...
    INSTPAT(L"│0000000     │?????│?????│011   │?????      │0110011│", R, rd = ((uint64_t)rs1 < (uint64_t)rs2) ? 1 : 0); // sltu // CHECK: zero-extend
    ...
    INSTPAT(L"│????????????      │?????│011   │?????      │0010011│", I, rd = (rs1 < imm) ? 1 : 0);              // sltiu // CHECK: zero-extend
    ...
    INSTPAT(L"│000000??????      │?????│101   │?????      │0010011│", I, rd = (uint64_t)rs1 >> BITS(imm, 4, 0)); // slli
    ...
    INSTPAT(L"│0000000?????      │?????│101   │?????      │0011011│", I, rd = SEXT(BITS((uint64_t)rs1 >> imm, 31, 0), 32));             // srliw
    INSTPAT(L"│0100000?????      │?????│101   │?????      │0011011│", I, rd = SEXT(BITS((int32_t)BITS(rs1, 31, 0) >> imm, 31, 0), 32)); // sraiw
    // S-type │imm[11:5]   │rs2  │rs1  │funct3│imm[4:0]   │opcode │
