...
static int decode_exec(Decode *s) {
    ...
    INSTPAT(L"│????????????      │?????│000   │?????      │1100111│", I, TODO());                // jalr
    INSTPAT(L"│????????????      │?????│011   │?????      │0000011│", I, rd = Mr(rs1 + imm, 8)); //  ld
    INSTPAT(L"│????????????      │?????│000   │?????      │0010011│", I, TODO());                //  addi
    INSTPAT(L"│000000000001      │00000│000   │00000      │1110011│", I, NEMUTRAP(PC, R(10)));   //  ebreak Note: R(10) is $a0
    // S-type │imm[11:5]   │rs2  │rs1  │funct3│imm[4:0]   │opcode │
    INSTPAT(L"│???????     │?????│?????│011   │?????      │0100011│", S, Mw(rs1 + imm, 8, rs2)); //  sd
    // B-type │imm[12|10:5]│rs2  │rs1  │funct3│imm[4:1|11]│opcode │
    // U-type │imm[31:12]                     │rd         │opcode │
    INSTPAT(L"│????????????????????           │?????      │0010111│", U, rd = PC + imm); //  auipc
    // J-type │imm[20|10:1|11|19:12]          │rd         │opcode │
    INSTPAT(L"│????????????????????           │?????      │1101111│", J, TODO()); //  jal
