...
static void decode_operand(Decode *s, int64_t **rd_pptr, int64_t **rs1_pptr, int64_t **rs2_pptr, int64_t *imm_ptr, int type) {
    ...
    switch (type) {
        ...
        case TYPE_U:
            *rd_pptr = (int64_t *)&R(rd);
            *imm_ptr = SEXT(BITS(i, 31, 12) << 12, 20); // sext(imm[31:12] << 12)
            break;
        case TYPE_J:
            *rd_pptr = (int64_t *)&R(rd);
...
static int decode_exec(Decode *s) {
    ...
    INSTPAT(L"│0000000     │?????│?????│010   │?????      │0110011│", R, rd = (rs1 < rs2) ? 1 : 0);                     // slt
    INSTPAT(L"│0000000     │?????│?????│011   │?????      │0110011│", R, rd = ((uint64_t)rs1 < (uint64_t)rs2) ? 1 : 0); // sltu // CHECK: as unsigned
    INSTPAT(L"│0000000     │?????│?????│100   │?????      │0110011│", R, rd = rs1 ^ rs2);                               // xor
    INSTPAT(L"│0000000     │?????│?????│101   │?????      │0110011│", R, rd = (uint64_t)rs1 >> rs2);                    // srl // CHECK: zero-extend
    INSTPAT(L"│0100000     │?????│?????│101   │?????      │0110011│", R, rd = rs1 >> rs2);                              // sra // CHECK: msb-extend
    ...
    INSTPAT(L"│????????????      │?????│001   │?????      │0000011│", I, rd = SEXT(BITS(Mr(rs1 + imm, 2), 15, 0), 16)); // lh
    INSTPAT(L"│????????????      │?????│010   │?????      │0000011│", I, rd = SEXT(BITS(Mr(rs1 + imm, 4), 31, 0), 32)); // lw
    INSTPAT(L"│????????????      │?????│100   │?????      │0000011│", I, rd = BITS(Mr(rs1 + imm, 1), 7, 0));            // lbu // CHECK: zero-extend
    INSTPAT(L"│????????????      │?????│101   │?????      │0000011│", I, rd = BITS(Mr(rs1 + imm, 2), 15, 0));           // lhu // CHECK: zero-extend
    INSTPAT(L"│????????????      │?????│000   │?????      │0010011│", I, rd = rs1 + imm);                               // addi
    INSTPAT(L"│????????????      │?????│011   │?????      │0010011│", I, rd = ((uint64_t)rs1 < (uint64_t)imm) ? 1 : 0); // sltiu // CHECK: as unsigned
    INSTPAT(L"│????????????      │?????│100   │?????      │0010011│", I, rd = rs1 ^ imm);                               // xori
    INSTPAT(L"│????????????      │?????│110   │?????      │0010011│", I, rd = rs1 | imm);                               // ori
    INSTPAT(L"│????????????      │?????│111   │?????      │0010011│", I, rd = rs1 & imm);                               // andi
    INSTPAT(L"│000000??????      │?????│001   │?????      │0010011│", I, rd = rs1 << BITS(imm, 5, 0));                  // slli // CHECK: shamt = imm[5:0]
    INSTPAT(L"│000000??????      │?????│101   │?????      │0010011│", I, rd = (uint64_t)rs1 >> BITS(imm, 5, 0));        // srli // CHECK: zero-extend,  shamt = imm[5:0]
    INSTPAT(L"│010000??????      │?????│101   │?????      │0010011│", I, rd = rs1 >> BITS(imm, 5, 0));                  // srai // CHECK: msb-extend,  shamt = imm[5:0]
    ...
    INSTPAT(L"│???????     │?????│?????│110   │?????      │1100011│", B, if ((uint64_t)rs1 < (uint64_t)rs2) { s->dnpc = s->pc + imm; }); // bltu // CHECK: as unsigned
    INSTPAT(L"│???????     │?????│?????│111   │?????      │1100011│", B, if ((uint64_t)rs1 >= (uint64_t)rs2) { s->dnpc = s->pc + imm; }); // bgeu // CHECK: as unsigned
    // U-type │imm[31:12]                     │rd         │opcode │
    INSTPAT(L"│????????????????????           │?????      │0110111│", U, rd = imm);         // lui
    ...
    // R-type │funct7      │rs2  │rs1  │funct3│rd         │opcode │
    INSTPAT(L"│0000001     │?????│?????│000   │?????      │0110011│", R, rd = rs1 * rs2);           // mul
    ...
    INSTPAT(L"│0000000     │?????│?????│001   │?????      │0111011│", R, rd = SEXT(BITS(rs1 << rs2, 31, 0), 32));          // sllw
    INSTPAT(L"│0000000     │?????│?????│101   │?????      │0111011│", R, rd = SEXT(BITS(rs1, 31, 0) >> rs2, 32));          // srlw // CHECK: zero-extend
    INSTPAT(L"│0100000     │?????│?????│101   │?????      │0111011│", R, rd = SEXT((int32_t)BITS(rs1, 31, 0) >> rs2, 32)); // sraw // CHECK: msb-extend
    // I-type │imm[11:0]         │rs1  │funct3│rd         │opcode │
    INSTPAT(L"│????????????      │?????│011   │?????      │0000011│", I, rd = Mr(rs1 + imm, 8));                                                    // ld
    INSTPAT(L"│????????????      │?????│000   │?????      │0011011│", I, rd = SEXT(BITS(rs1 + imm, 31, 0), 32));                                    // addiw
    INSTPAT(L"│0000000?????      │?????│001   │?????      │0011011│", I, rd = (uint64_t)(int32_t)BITS(rs1 << BITS(imm, 4, 0), 31, 0));                       // slliw // CHECK: shamt = imm[4:0]
    INSTPAT(L"│0000000?????      │?????│101   │?????      │0011011│", I, rd = (uint64_t)(int32_t)BITS((uint64_t)rs1 >> BITS(imm, 4, 0), 31, 0));             // srliw // CHECK: zero-extend, shamt = imm[4:0]
    INSTPAT(L"│0100000?????      │?????│101   │?????      │0011011│", I, rd = (uint64_t)(int32_t)((int32_t)BITS(rs1, 31, 0) >> BITS(imm, 4, 0))); // sraiw // CHECK: msb-extend, shamt = imm[4:0]
    // S-type │imm[11:5]   │rs2  │rs1  │funct3│imm[4:0]   │opcode │
    ...
    // R-type │funct7      │rs2  │rs1  │funct3│rd         │opcode │
    INSTPAT(L"│0000001     │?????│?????│000   │?????      │0111011│", R, rd = SEXT(BITS(rs1 * rs2, 31, 0), 32)); // mulw
    INSTPAT(L"│0000001     │?????│?????│100   │?????      │0111011│", R, rd = SEXT((int32_t)BITS(rs1, 31, 0) / (int32_t)BITS(rs2, 31, 0), 32)); // divw
    INSTPAT(L"│0000001     │?????│?????│110   │?????      │0111011│", R, rd = SEXT((int32_t)BITS(rs1, 31, 0) % (int32_t)BITS(rs2, 31, 0), 32)); // remw
