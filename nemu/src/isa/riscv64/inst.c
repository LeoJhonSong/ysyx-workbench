...
static int decode_exec(Decode *s) {
    ...
    INSTPAT(L"│000000??????      │?????│001   │?????      │0010011│", I, rd = rs1 << BITS(imm, 4, 0));                  // slli
    INSTPAT(L"│000000??????      │?????│101   │?????      │0010011│", I, rd = (uint64_t)rs1 >> BITS(imm, 4, 0));        // slli // CHECK: zero-extend
    INSTPAT(L"│010000??????      │?????│101   │?????      │0010011│", I, rd = rs1 >> BITS(imm, 4, 0));                  // srai // CHECK: msb-extend
    ...
    INSTPAT(L"│???????     │?????│?????│110   │?????      │1100011│", B, if ((uint64_t)rs1 < (uint64_t)rs2) { s->dnpc = s->pc + imm; }); // bltu // CHECK: zero-extend
    INSTPAT(L"│???????     │?????│?????│111   │?????      │1100011│", B, if ((uint64_t)rs1 >= (uint64_t)rs2) { s->dnpc = s->pc + imm; }); // bgeu // CHECK: zero-extend
    ...
    INSTPAT(L"│0000000     │?????│?????│101   │?????      │0111011│", R, rd = SEXT(BITS(rs1, 31, 0) >> BITS(rs2, 4, 0), 32));          // srlw // CHECK: zero-extend
    INSTPAT(L"│0100000     │?????│?????│101   │?????      │0111011│", R, rd = SEXT((int32_t)BITS(rs1, 31, 0) >> BITS(rs2, 4, 0), 32)); // sraw // CHECK: msb-extend
    // I-type │imm[11:0]         │rs1  │funct3│rd         │opcode │
